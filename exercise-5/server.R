...
shinyServer(function(input, output) {
  ...
  output$plotNamehere <- renderPlot({
    ...
    if (input$sex == 'Men')
      data <- filter(womens.role, sex == 'Male')
    else if (input$sex == 'Women')
      data <- filter(womens.role, sex == 'Female')
    else data <- womens.role
  
    
    #Define a y axis value based on the input Agree or Disagree
    if (input$thoughts == 'Agree')
      thoughts = data$agree
    else thoughts = data$disagree
    ...
    ggplot(data, aes(x=education, y=thoughts, color=factor(sex))) + geom_point()
